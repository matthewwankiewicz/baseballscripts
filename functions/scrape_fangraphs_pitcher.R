...
fg_pitcher_leaders <- function(
  ...
  resp <- httr::GET(fg_endpoint)
  
  if (httr::status_code(resp) == 200) {
    data <- httr::content(resp, "text", encoding = "UTF-8")
    parsed <- jsonlite::fromJSON(data, flatten = TRUE)
    return(parsed$data)
  } else {
    message(paste("Error:", httr::status_code(resp)))
    message(httr::content(resp, "text", encoding = "UTF-8"))
    return(NULL)
  }
}
